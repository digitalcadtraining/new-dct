...
export default function Header({ onMenuToggle }) {
  ...
  // Hide public marketing nav on all authenticated dashboard pages
  const isDashboard =
  ...
  return (
    <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-4 sm:px-6 flex-shrink-0 z-30">
      <div className="flex items-center gap-3">
        ...
        {/* Public nav — hidden on dashboard routes */}
        {!isDashboard && (
          <nav className="hidden md:flex items-center gap-6">
            {["Home","About","Courses","Contact Us","Tutor"].map(item => (
              <a key={item} href="#"
                className="text-sm font-semibold text-dct-dark hover:text-dct-primary transition-colors">
                {item}
              </a>
            ))}
          </nav>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={ref}>
          <button onClick={() => setOpen(v => !v)}
            ...
            <span className="hidden sm:block text-sm font-semibold text-dct-dark">
              {user?.name || "User"}
            </span>
