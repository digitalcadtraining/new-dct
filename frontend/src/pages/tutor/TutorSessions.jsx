...
function UploadAssignmentModal({ session, batchId, existingAssignment, onClose, onUploaded }) {
  ...
  const defaultTitle = session.name || `Session ${session.session_number} Assignment`;
  ...
  const [title,     setTitle]     = useState(existingAssignment?.title || defaultTitle);
  ...
  const handleSubmit = async () => {
    ...
    if (!file && !existingAssignment?.file_url) return setErr("Please attach a file — students need it.");
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div>
                <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.gray,
                  ...
                  Title
                  <span style={{ fontWeight:400, marginLeft:6, color:C.lg }}>(auto-filled)</span>
                </label>
                {editTitle ? (
                  <input type="text" value={title} onChange={e=>setTitle(e.target.value)}
                    autoFocus
                    style={{ width:"100%", padding:"10px 13px", border:`1.5px solid ${C.primary}`,
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 13px",
                    border:"1.5px solid #e5e7eb", borderRadius:11, background:"#f8fafc",
                    cursor:"pointer" }} onClick={()=>setEditTitle(true)}>
                    <span style={{ flex:1, fontSize:13, color:C.dark }}>{title}</span>
                    <span style={{ fontSize:10, color:C.primary, fontWeight:700, flexShrink:0 }}>✏ Edit</span>
                  </div>
                )}
              </div>
              <div>
              ...
              <div>
                <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.gray,
                  marginBottom:5, textTransform:"uppercase", letterSpacing:"0.5px" }}>
                  Assignment File *
                  <span style={{ fontWeight:400, marginLeft:4 }}>(required — students download this)</span>
                </label>
                {file ? (
                  <div style={{ display:"flex", alignItems:"center", gap:8, padding:"11px 13px",
                    ...
                    <FileText size={14} style={{ color:C.primary, flexShrink:0 }}/>
                  ...
                  <button onClick={()=>fileRef.current?.click()}
                    style={{ width:"100%", padding:"18px 0", border:"2px dashed #d1d5db",
                      borderRadius:11, background:"#fafafa", cursor:"pointer", boxSizing:"border-box",
                      display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                    <Upload size={16} style={{ color:C.lg }}/>
                    <span style={{ fontSize:12, color:C.lg }}>Click to attach assignment file</span>
                  </button>
                )}
              ...
              <div>
                <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.gray,
                  marginBottom:5, textTransform:"uppercase", letterSpacing:"0.5px" }}>
                  Due Date <span style={{ fontWeight:400 }}>(auto: this Sunday 11PM)</span>
                </label>
                <input type="date" value={due} onChange={e=>setDue(e.target.value)}
                  style={{ width:"100%", padding:"10px 13px",
                    ...
                    color: due ? (isLate ? "#dc2626" : C.dark) : C.lg, boxSizing:"border-box" }}/>
                {isLate && (
                  <p style={{ fontSize:10, color:"#dc2626", marginTop:3, fontWeight:600 }}>
                    ⚠ Past date — submissions will be marked Late
                  </p>
                )}
              </div>
              {err && (
              ...
              <button onClick={handleSubmit} disabled={loading||!title.trim()||(!file&&!existingAssignment?.file_url)}
                style={{ width:"100%", padding:"13px 0",
                  background: (!title.trim()||(!file&&!existingAssignment?.file_url)) ? "#e5e7eb" : `linear-gradient(135deg,${C.blue},${C.primary})`,
                  color: (!title.trim()||(!file&&!existingAssignment?.file_url)) ? C.lg : "#fff",
