...
function UploadAssignmentModal({
  ...
  // Auto due date: next Sunday at 23:00
  function nextSunday11PM() {
    const d = new Date();
  ...
  // Show late warning if due date in past
  const isLate = due && new Date(due) < new Date();
        ...
        <div style={{ padding: "20px 22px" }}>
          ...
          {done ? (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              ...
              <p style={{ fontSize: 12, color: C.lg, marginTop: 4 }}>
                Students can now see and submit this.
              </p>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                <div>
                  ...
                  >
                    Due Date{" "}
                    <span style={{ fontWeight: 400, color: C.lg }}>
                      (auto: this Sunday 11PM)
                    </span>
                  </label>
                  <input
                    type="date"
                    ...
                    style={{
                      width: "100%",
                      padding: "10px 13px",
                      border: `1.5px solid ${isLate ? "#fca5a5" : "#e5e7eb"}`,
                      borderRadius: 11,
                      fontSize: 12,
                      outline: "none",
                      fontFamily: "inherit",
                      color: due ? (isLate ? "#dc2626" : C.dark) : C.lg,
                      boxSizing: "border-box",
                    }}
                    onFocus={(e) => (e.target.style.borderColor = C.primary)}
                    onBlur={(e) =>
                      (e.target.style.borderColor = isLate
                        ? "#fca5a5"
                        : "#e5e7eb")
                    }
                  />
                  {isLate && (
                    ...
                    >
                      ⚠ This date is in the past — submissions will be marked
                      Late
                    </p>
                  )}
                </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Upload size={14} />{" "}
                    {isEdit ? "Save Changes" : "Upload Assignment"}
                  </>
                )}
              </button>
            </div>
  ...
  const handleSave = async () => {
    ...
    try {
      await sessionApi.update(session.id, {
        [isComp ? "recording_url" : "zoom_link"]: url.trim(),
      });
      onSaved(url.trim());
      onClose();
    } catch (e) {
    ...
    >
      ...
      >
        ...
        <input
          ...
          placeholder={
            isComp ? "https://zoom.us/rec/..." : "https://zoom.us/j/..."
          }
          style={{
            width: "100%",
            padding: "11px 14px",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h3 style={{ fontWeight: 800, color: C.dark, fontSize: 16 }}>
              Answer Query
            </h3>
            <p style={{ fontSize: 11, color: C.lg, marginTop: 2 }}>
              {query.student?.name} · Session{" "}
              {query.session?.session_number || "—"} ·{" "}
        ...
        >
          ...
          <p style={{ fontSize: 13, color: C.dark, lineHeight: 1.6 }}>
            {query.question}
          </p>
        </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        transition: "all 0.2s",
        boxShadow: isSel
          ? `0 4px 18px rgba(0,123,191,0.15)`
      ...
      <div style={{ padding: "14px 20px" }}>
        ...
        {isComp ? (
          session.recording_url ? (
            <a
              href={mediaUrl(session.recording_url) || session.recording_url}
              target="_blank"
        ...
        {assignment ? (
          ...
          >
            ...
            <button
              onClick={() => onAssignmentUpload(session, assignment)}
              style={{
                padding: "9px 12px",
                background: "#fff",
          ...
          <button
            onClick={() => onAssignmentUpload(session, null)}
            style={{
              width: "100%",
              marginTop: 8,
              padding: "9px 0",
        ...
        <button
          onClick={() => onViewSessionQueries(session)}
          style={{
            width: "100%",
            marginTop: 6,
            padding: "8px 0",
...
export default function TutorSessionsPage() {
  ...
  const ultraCount = queries.filter(
    (q) => getQueryPriority(q) === "ultra",
  ).length;
  ...
  const handleViewSessionQueries = (session) => {
    navigate(
      `/tutor/queries?session_id=${session.id}&session_num=${session.session_number}&batch_id=${selectedBatch?.id || ""}`,
    );
  };
  ...
  const assignmentBySession = useMemo(() => {
    ...
    assignments.forEach((a) => {
      if (a.session_id) map[a.session_id] = a;
    });
    return map;
  }, [assignments]);
  ...
  const handleAnswered = (queryId) => {
    setQueries((q) =>
      q.map((x) => (x.id === queryId ? { ...x, status: "RESOLVED" } : x)),
    );
  };
  const handleLinkSaved = (sessionId, url) => {
    const sess = sessions.find((s) => s.id === sessionId);
    const field =
      sess?._derivedStatus === "COMPLETED" ? "recording_url" : "zoom_link";
    setSessions((prev) =>
      prev.map((s) => (s.id === sessionId ? { ...s, [field]: url } : s)),
    );
  };

  const cardProps = {
    timeSlots,
    onLinkModal: setLinkModalSession,
    onAssignmentUpload: (session, assignment) =>
      setAssignmentModal({ session, assignment }),
        ...
        {!loadingBatches && batches.length === 0 && (
          ...
          >
            ...
            <p style={{ fontSize: 13, color: C.lg }}>
              Create a batch and wait for admin approval.
            </p>
          </div>
        )}
        {!loadingBatches && batches.length > 0 && (
                ...
                {FILTERS.filter((f) => !f.hide).map(({ key, label, count }) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow:
                        filter === key
                          ? `0 4px 14px rgba(0,123,191,0.25)`
                          : "none",
                    }}
                  >
                    {label}
                      ...
                      >
                        {grouped.live.map((s, i) => (
                          <SessionCard
                            ...
                            session={s}
                            index={i}
                            sessionQueryCount={sessionQueryCounts[s.id] || 0}
                            {...cardProps}
                          />
                        ))}
                      </div>
              ...
              >
                ...
                >
                  {[
                    ...
                    {
                      val: enriched.length - completedCount,
                      label: "Left",
                      bg: "#f9fafb",
                      color: C.dark,
                    },
                    ...
                    >
                      ...
                      <p style={{ fontSize: 10, fontWeight: 600, color: C.lg }}>
                        {label}
                      </p>
                    </div>
                  ))}
          ...
          {assignmentModal && (
            <UploadAssignmentModal
              ...
              onUploaded={() => {
                ...
                if (selectedBatch) {
                  assignmentApi
                    .getTutorBatch(selectedBatch.id)
