...
function Skeleton({ cls = "" }) {
  return <div className={`rounded-2xl bg-gray-100 animate-pulse ${cls}`} />;
}

function StatCard({ icon: Icon, label, value, sub, color, onClick, delay = 0, loading }) {
  if (loading) return <Skeleton cls="h-28" />;
  return (
    <motion.div
...
function Panel({ title, children, onViewAll, delay = 0, loading, empty }) {
  return (
    <motion.div
      ...
      {loading
        ? <div className="p-5 space-y-3">{[1,2,3].map(i=><Skeleton key={i} cls="h-12"/>)}</div>
        : empty
