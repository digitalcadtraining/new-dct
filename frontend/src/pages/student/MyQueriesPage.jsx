...
import {
  ...
  Clock, Image, Video, AlertCircle, RefreshCw, MessageCircle,
} from "lucide-react";
...
function AskSheet({ batches, selectedBatchId, sessionId, sessionNum, queryNumber, onClose, onSubmitted }) {
          ...
          {batches.length > 1 && (
            <div style={{ marginBottom:14 }}>
              <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.gray,
                marginBottom:5, textTransform:"uppercase", letterSpacing:"0.5px" }}>Batch</label>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            {preview ? (
              <div style={{ position:"relative", display:"inline-block" }}>
                {isVideo
                  ? <video src={preview} controls style={{ maxWidth:"100%", maxHeight:150,
                      borderRadius:12, display:"block", border:"1px solid #e5e7eb" }}/>
                  : <img src={preview} alt="preview"
                      style={{ maxWidth:"100%", maxHeight:160, borderRadius:12,
              ...
              <button onClick={()=>fileRef.current?.click()}
                style={{ width:"100%", padding:"16px 14px", border:"2px dashed #d1d5db",
          ...
          <button onClick={handleSubmit} disabled={loading || !question.trim()}
            style={{ width:"100%", padding:"15px 0",
              background: loading || !question.trim()
                ? "#e5e7eb" : `linear-gradient(135deg,${C.blue},${C.primary})`,
    ...
    <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
        ...
        <span style={{ fontSize:10, fontWeight:700, padding:"3px 8px", borderRadius:999,
          background: localResolved ? "#f0fdf4" : hasAnswer ? "#eff8ff" : "#fff7ed",
          color: localResolved ? "#16a34a" : hasAnswer ? C.primary : "#ea580c" }}>
          {localResolved ? "✓ Resolved" : hasAnswer ? "Answered" : "Waiting"}
        </span>
      </div>
      ...
      <div style={{ background:"#fff", borderRadius:"4px 18px 18px 18px",
        ...
        boxShadow:"0 2px 8px rgba(0,0,0,0.04)", marginBottom: hasAnswer ? 8 : 0 }}>
        ...
        {attachUrl && (
          <div style={{ marginTop:10, borderRadius:12, overflow:"hidden",
            maxWidth:240, border:"1px solid #e5e7eb" }}>
            {isVideo
      ...
      {hasAnswer && (
        <div style={{ marginLeft:24 }}>
          <div style={{ background:"linear-gradient(135deg,#eff8ff,#e0f0ff)",
            ...
            border:"1px solid #bfdbfe", padding:"13px 15px",
      ...
      <PageWrapper>
        ...
        {queries.length > 0 && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)",
            ...
            {[
              { val:openCount,     label:"Waiting",  color:"#ea580c", bg:"#fff7ed" },
              { val:answeredCount, label:"Answered",  color:C.primary, bg:"#eff8ff" },
              { val:resolvedCount, label:"Resolved",  color:"#16a34a", bg:"#f0fdf4" },
        ...
        {loading && (
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            {[1,2].map(i=>(
              <div key={i} style={{ height:160, background:"#f3f4f6",
        ...
        {/* Empty */}
        {!loading && queries.length === 0 && <EmptyState onAsk={() => setAskOpen(true)}/>}

        {/* Empty filtered */}
        {!loading && queries.length > 0 && displayed.length === 0 && (
          <div style={{ textAlign:"center", padding:"40px 0" }}>
            <p style={{ fontSize:14, color:C.lg }}>No {filter.toLowerCase()} queries</p>
          </div>
        )}
