  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 style={{ fontSize: 17, fontWeight: 700, color: C.dark }}>
            Ask a Question
          </h3>
          <button
            onClick={onClose}
      ...
      <div style={{ padding: "14px 20px" }}>
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: 10, color: C.primary }}>
              Mentor · Live Session
            </p>
          </div>
        </div>
        ...
        >
          ...
          {sessionQueryCount > 0 ? (
            <button
              onClick={() => onViewMyQueries(session)}
              style={{
                display: "flex",
                alignItems: "center",
                justifyContent: "space-between",
...
function BatchSelectorCard({ batch, selected, onClick }) {
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 11, color: C.lg, marginBottom: 4 }}>
        {batch.course?.name || ""}
      </p>
...
function useBatchSessions() {
  ...
  const [sessionAssignments, setSessionAssignments] = useState({}); // sessionId → assignment
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      import("../../services/api.js")
        .then((m) => m.assignmentApi.getForBatch(selectedBatch.id))
        .catch(() => ({ data: [] })),
    ])
  ...
  const sessionQueryCounts = useMemo(() => {
    const map = {};
    myQueries
      .filter((q) => q.status !== "RESOLVED" && q.status !== "AUTO_RESOLVED")
      .forEach((q) => {
...
export function AllSessionsPage() {
  ...
  const handleViewMyQueries = (session) => {
    navigate(
      `/student/queries?session_id=${session.id}&session_num=${session.session_number}`,
    );
  };
      ...
      <PageWrapper>
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 13, color: C.lg, marginTop: 2 }}>
            Track all your learning sessions across batches
          </p>
        </div>
        ...
        {!loadingBatches && batches.length === 0 && (
          ...
          >
            ...
            <p style={{ fontSize: 13, color: C.lg }}>
              Enroll in a course to see your sessions here.
            </p>
          </div>
        )}
          ...
          <>
            ...
            {enriched.length > 0 && (
              ...
              >
                ...
                >
                  <span
                    style={{ fontSize: 13, fontWeight: 700, color: C.dark }}
                  >
                    Course Completion
                  </span>
                  <span
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    ...
                    >
                      {grouped.live.map((s, i) => (
                        <SessionCard
                          ...
                          onViewMyQueries={handleViewMyQueries}
                          assignment={sessionAssignments[s.id] || null}
                          index={i}
                        />
                      ))}
                    </div>
                ...
                {enriched.length === 0 && (
                  ...
                  >
                    ...
                    <p style={{ fontSize: 13, color: C.lg }}>
                      Sessions will appear once scheduled by your tutor.
                    </p>
                  </div>
                )}
              </div>
...
export function UpcomingSessionsPage() {
  ...
  const filtered = useMemo(
    () =>
      enriched.filter(
        (s) =>
          s._derivedStatus === "UPCOMING" ||
          s._derivedStatus === "TODAY" ||
          s._derivedStatus === "LIVE",
      ),
    [enriched],
  );
  const tutorName = selectedBatch?.tutor?.name || "Tutor";
    ...
    <AppShell>
      <PageWrapper>
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 13, color: C.lg, marginTop: 2 }}>
            {filtered.length} sessions scheduled
          </p>
        </div>
        {!loadingBatches && (
          <BatchSelector
        ...
        {loadingSessions && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[1, 2].map((i) => (
              <div
                key={i}
                style={{ height: 260, background: "#f3f4f6", borderRadius: 16 }}
                className="animate-pulse"
              />
            ))}
          </div>
        ...
        {!loadingSessions && filtered.length === 0 && selectedBatch && (
          ...
          >
            ...
            />
            <p style={{ fontSize: 14, color: C.lg }}>
              No upcoming sessions for this batch
            </p>
          </div>
        )}
...
export function CompletedSessionsPage() {
  ...
  const filtered = useMemo(
    () => enriched.filter((s) => s._derivedStatus === "COMPLETED"),
    [enriched],
  );
  const tutorName = selectedBatch?.tutor?.name || "Tutor";
  const timeSlots = selectedBatch?.time_slots || [];
    ...
    <AppShell>
      <PageWrapper>
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 13, color: C.lg, marginTop: 2 }}>
            {filtered.length} sessions completed
          </p>
        </div>
        {!loadingBatches && (
          <BatchSelector
