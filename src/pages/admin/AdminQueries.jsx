/**
 * AdminQueries — fully live, real API
 * Features: filter by status, query detail modal, real resolve action, toast
 */
        ...
        <motion.div
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="bg-gray-50 rounded-2xl p-4">
              <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color:C.lg }}>
                Student's Question
              </p>
            ...
            {attachUrl && (
              <div className="rounded-2xl overflow-hidden border border-gray-100">
                ...
                <p className="text-xs px-3 py-1.5 bg-gray-50" style={{ color:C.lg }}>
                  📎 Student attachment
                </p>
              </div>
            )}
            ...
            {query.answer ? (
              <div className="rounded-2xl p-4 border border-blue-100"
                ...
                {query.answered_at && (
                  <p className="text-xs mt-2" style={{ color:C.lg }}>Answered {fmtRelative(query.answered_at)}</p>
                )}
              </div>
            ) : (
            ...
            {!isResolved && (
              <button onClick={() => onResolve(query)}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-white font-bold text-sm transition-all hover:-translate-y-0.5"
...
export default function AdminQueries() {
  ...
  const handleResolve = async (query) => {
    try {
      ...
      setToast({ msg: e.message || "Failed to resolve query", type:"error" });
    }
  };
