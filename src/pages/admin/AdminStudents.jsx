/**
 * AdminStudents — fully live, real API
 * Features: search, pagination, active/inactive toggle, student detail drawer
 */
import { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
  Search, Users, ChevronLeft, ChevronRight,
  X, ToggleLeft, ToggleRight, BookOpen,
} from "lucide-react";
              ...
              {enrollments.length === 0
                ...
                : enrollments.map((e, i) => (
                    <div key={i} className="p-4 rounded-2xl border border-gray-100 bg-white mb-3"
                      ...
                      <p className="text-xs" style={{ color:C.lg }}>
                        Tutor: {e.batch?.tutor?.name || "—"}
                      </p>
                      <p className="text-xs mt-1" style={{ color:C.lg }}>
                        Enrolled: {fmt(e.enrolled_at)}
                      </p>
                    </div>
                  ))
              }
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden"
          ...
          <div className="grid grid-cols-12 gap-3 px-5 py-3 border-b border-gray-100 bg-gray-50/60">
            {["Student","Batch / Course","Tutor","Joined","Status"].map((h,i)=>(
              <p key={h} className={`text-[11px] font-extrabold uppercase tracking-wider
