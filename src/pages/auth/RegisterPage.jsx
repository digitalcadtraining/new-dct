...
export default function RegisterPage() {
  ...
  // Pre-selected course: URL may send slug or course_id
  const slugFromUrl   = searchParams.get("course");     // e.g. "plastic-product-design"
  const courseIdFromUrl = searchParams.get("course_id"); // e.g. uuid
  ...
  // Whether course was pre-selected (lock the dropdown)
  ...
  // Once courses load, match slug → course_id
  useEffect(() => {
  ...
  // Load batches when course_id changes
  useEffect(() => {
    if (!form.course_id) { setBatches([]); return; }
    courseApi.getBatches(form.course_id)
  ...
  const validate = () => {
    ...
    if (!form.batch_id)       return batches.length === 0
            ...
            {step === 1 && (
              <motion.div key="step1"
                ...
                <div className="space-y-4">
                  ...
                  <Input label="Email Address" type="email" placeholder="rahul@gmail.com"
                    value={form.email} onChange={e => update("email", e.target.value)} />
                  <Input label="Phone Number" type="tel" placeholder="98765 43210"
                    value={form.phone} onChange={e => update("phone", e.target.value)} />

                  {/* Course selector — locked if pre-selected */}
                  <div>
                    <label className="block text-xs font-semibold text-dct-gray mb-1.5 uppercase tracking-wider">
                      Select Course
                    </label>
                    {courseLocked && selectedCourse ? (
                      /* Locked — show as info box */
                      <div className="dct-input w-full flex items-center justify-between"
                        style={{ background: "#f0f7ff", borderColor: "#bfdbfe", color: "#024981" }}>
                  ...
                  {/* Batch selector — only shows when course is selected */}
                  {form.course_id && (
                    <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:"auto" }}>
                      ...
                      {batches.length === 0 ? (
                        ...
                        <select value={form.batch_id} onChange={e => update("batch_id", e.target.value)}
                          className="dct-input w-full">
                      ...
                      {form.batch_id && (() => {
                        ...
                        return (
                          <div className="mt-2 p-3 rounded-xl bg-blue-50 border border-blue-100 text-xs text-dct-gray space-y-1">
                            <p><strong>Starts:</strong> {new Date(b.start_date).toLocaleDateString("en-IN", { day:"numeric", month:"long", year:"numeric" })}</p>
                            {b.time_slots?.length > 0 && <p><strong>Timing:</strong> {b.time_slots.join(", ")}</p>}
                            <p><strong>Tutor:</strong> {b.tutor_name || "Industry Expert"}</p>
                          </div>
                        );
                      })()}
                    </motion.div>
                ...
                <p className="text-center text-sm text-dct-gray mt-5">
                  ...
                  <Link to="/auth/login" className="text-dct-primary font-bold hover:underline">Sign In</Link>
                </p>
              </motion.div>
            )}
                  ...
                  <div className="flex justify-center gap-2">
                    {otp.map((digit, idx) => (
                      <input key={idx} id={`otp-${idx}`} type="text" inputMode="numeric" maxLength={1}
                        ...
                        style={{ borderColor: digit ? "#007BBF" : "#e5e7eb", color: "#1F1A17" }} />
                    ))}
                  </div>
